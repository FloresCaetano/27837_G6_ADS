
# Linea Base LBR - Requisitos

Carpeta que contiene la especificacion completa de requisitos del sistema Kairos Mix.

## Contenido principal:
- Documento SRS (Especificacion de Requisitos de Software)
- Historias de Usuario aprobadas
- Matriz de Trazabilidad IREB
- Requisitos Funcionales y No Funcionales validados

## Requisitos Destacados:
- Gestion de Productos, Clientes y Pedidos
- **Diseñador de Mezclas Personalizadas**
- **Calculo Nutricional Automatico (NutricionalService)**

## Fecha de inicio:
14 de enero de 2026

## Responsables:
Caetano Flores, Jordan Guaman, Anthony Morales, Leonardo Narvaez

